...
def okx_sign(ts,method,path,body=""):
    return base64.b64encode(hmac.new(SECRET_KEY.encode(),( ts+method+path+body).encode(),hashlib.sha256).digest()).decode()

def okx_hdrs(method,path,body=""):
    ts=okx_ts()
    return {"OK-ACCESS-KEY":API_KEY,"OK-ACCESS-SIGN":okx_sign(ts,method,path,body),
            "OK-ACCESS-TIMESTAMP":ts,"OK-ACCESS-PASSPHRASE":PASSPHRASE,
            "Content-Type":"application/json","x-simulated-trading":"1"}

def okx_get(path):
    try: return requests.get(BASE_URL+path,headers=okx_hdrs("GET",path),timeout=10).json()
    except Exception as e: return {"code":"-1","msg":str(e)}

def okx_post(path,body):
    b=json.dumps(body)
    try: return requests.post(BASE_URL+path,headers=okx_hdrs("POST",path,b),data=b,timeout=15).json()
    except Exception as e: return {"code":"-1","msg":str(e)}
...
def place_spot(sym,side,qty_usdt):
    price=get_price(sym)
    if price<=0: return None,0
    qty=round(qty_usdt/price,6)
    if qty*price<1: return None,0
    r=okx_post("/api/v5/trade/order",{
        "instId":sym,"tdMode":"cash","side":side,
        "ordType":"market","sz":str(qty)
    })
    if r.get("code")=="0":
        return r["data"][0].get("ordId","OK"),qty
    tg("⚠️ فشل "+side+" "+sym+": "+str(r.get("msg",""))+" كود:"+str(r.get("data","")))
    return None,0

def close_spot(sym,side,qty):
    r=okx_post("/api/v5/trade/order",{
        "instId":sym,"tdMode":"cash","side":side,
        "ordType":"market","sz":str(qty)
    })
...
def check_risk():
    ...
    if last_date!=today:
        ...
        if carried>0: tg("🌅 يوم جديد | "+str(carried)+" مفتوحة | مسموح "+str(allowed_new_today))
    if usdt-daily_start_eq<=-MAX_DAILY_LOSS:
        stopped_out=True; stop_reason="تجاوز حد الخسارة اليومي 250$"; tg("⛔ "+stop_reason); return False
    if usdt<=CAPITAL-MAX_TOTAL_LOSS:
        stopped_out=True; stop_reason="تجاوز حد الخسارة التراكمي 500$"; tg("⛔ "+stop_reason); return False
    return True
...
def analyze(sym):
    ...
    cl=[x["c"] for x in kl];hi=[x["h"] for x in kl];lo=[x["l"] for x in kl];vl=[x["v"] for x in kl]
    highest=max(hi[-LOOKBACK-2:-2]); lowest=min(lo[-LOOKBACK-2:-2])
    a=atr(hi[:-1],lo[:-1],cl[:-1]); d=adx(hi[:-1],lo[:-1],cl[:-1])
...
def report(cycle):
    if stopped_out: tg("⛔ <b>متوقف</b>: "+stop_reason); return
    ...
    if positions:
        ...
        for p in positions:
            ...
            if cur>0:
                ...
                msg+="\n"+("🟢" if unr>=0 else "🔴")+" "+("Long" if p["dir"]=="Long" else "Short")+" "+p["sym"].replace("-USDT","")
...
def demo_trade():
    ...
    oid,qty=place_spot("BTC-USDT","buy",11.0)
    if oid:
        tg("✅ <b>تم فتح الصفقة التجريبية!</b>\n🟢 Long BTC-USDT\n📊 دخول: "+"%.2f"%price+"$\n📦 كمية: "+str(qty)+" BTC\n⏳ سيتم الإغلاق بعد 3 دقائق...")
        time.sleep(180)
        exit_price=get_price("BTC-USDT")
        ok=close_spot("BTC-USDT","sell",qty)
        if ok:
            diff=exit_price-price if exit_price>0 else 0
    ...
    try:
        ...
        for sym in SYMBOLS:
            ...
            try:
                ...
                if sig and len(positions)<MAX_OPEN and not already and opened_today<allowed_new_today:
                    ...
                    if dist<=0: continue
                    qty_usdt=FIXED_RISK
                    oid,qty=place_spot(sym,"buy" if sig["dir"]=="Long" else "sell",qty_usdt)
                    if oid:
                        positions.append({"sym":sym,"dir":sig["dir"],"entry":price,"stop":sig["stop"],"target":sig["target"],"qty":qty})
                        opened_today+=1
        ...
        for pos in list(positions):
            ...
            if hit:
                ...
                tg(("✅ ربح" if net>0 else "❌ خسارة")+" | "+("Long" if pos["dir"]=="Long" else "Short")+" "+pos["sym"].replace("-USDT","")+"\n📊 دخول: "+"%.4f"%pos["entry"]+"\n📊 خروج: "+"%.4f"%hit+"\n💰 صافي: "+"%+.2f"%net+"$ | رصيد: "+"%.2f"%usdt+"$")
                check_risk()
        if cycle%15==0: report(cycle)
